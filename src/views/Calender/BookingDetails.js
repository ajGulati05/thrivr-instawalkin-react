...
import { cancelBookingReqAction, getBookingDetailsReqAction } from "../../store/actions/bookings";
import { useDispatch, useSelector } from "react-redux";
import { isEmpty } from "lodash";
import CovidForm from "../clients/CovidForm";
import { useHistory } from "react-router-dom";

const { Panel } = Collapse;

const BookingDetails = ({ handleBookingDetails, handleModifyBooking, handleCancelBooking, handleViewInSchedule }) => {
  ...
  useEffect(() => {
    if (confirmCancel.data) {
      dispatch(getBookingDetailsReqAction(bookingDetails.slug));
    }
  }, [confirmCancel]);
  ...
  const { client, project,pricing } = bookingDetails;
          ...
          <Col span={24}>
            <Row gutter={16}>
              ...
              <Col span={24}>
                <br />
                <PButton
                  pname="View in Schedule"
                  onClick={() => handleViewInSchedule("changeView", bookingDetails.start_datetime)}
                  width="100%"
                />
              </Col>
              <Col span={24}>
                <Collapse defaultActiveKey={["1"]} expandIconPosition="right" ghost>
                  <Panel header="Booking Info" key="1">
                    <Card bordered={false}>
                      <div>
                        <h5 onClick={handleClientClick} className="client-name">
                          {client.name}
                        </h5>
                        <span>
                  ...
                  <Panel header="Billing Info" key="2">
                    <Card bordered={false}>
                      ...
                      {project && (
                        <div>
                          <span>
                            Amount  ({project.description}) : <b>${pricing.amount}</b>
                          </span>
                          <br />
                          <span>
                            Tax ({project.pricing.taxlabel}): <b>${pricing.tax_amount}</b>
                          </span>
                          <br />
                          {pricing.discount && (
                            <>
                              <span>
                                Discount: <b>${pricing.discount}</b>
                              </span>
                              <br />
                            </>
                          )}
                          ...
                          <span>
                            ...
                            {bookingDetails.pricing.tip_amount ? (
                              <b>
                                ${parseInt(bookingDetails.total_amount) + parseInt(bookingDetails.pricing.tip_amount)}
                              </b>
                            ) : (
                              <b>${bookingDetails.total_amount}</b>
                            )}
                          </span>
                          <Divider />
                          <span>
                            {bookingDetails.paid_by_description} : <b>${bookingDetails.splitPricing.amount_1}</b>
                          </span>
                          {bookingDetails.paid_by_2_description && (
                            <>
                              <Divider />
                              <span>
                                {bookingDetails.paid_by_2_description} : <b>${bookingDetails.splitPricing.amount_2}</b>
                              </span>
                            </>
                          )}
                        </div>
                  ...
                  <Panel header="Covid Form" key="3" className="booking-covid">
                    {bookingDetails.covid_form && (
                      <CovidForm covidFormDetail={bookingDetails.covid_form} isBookingDetails={true} />
                    )}
                  </Panel>
                </Collapse>
              </Col>
